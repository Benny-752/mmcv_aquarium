...
def parse_meta_file():
    ...
    for meta_file_path in all_meta_file_paths:
        ...
        # Check if 'Models' exists in the YAML file
        if 'Models' not in yaml_file:
            continue  # Skip if no 'Models' key

        for i in range(len(yaml_file['Models'])):
            model_data = yaml_file['Models'][i]

            # Check for 'Weights' key
            if 'Weights' in model_data and isinstance(model_data['Weights'], str):
                weights_list.append(model_data['Weights'])
            elif 'Results' in model_data and len(model_data['Results']) > 0:
                # Check for 'Metrics' and 'Weights' keys
                metrics = model_data['Results'][0].get('Metrics', {})
                if 'Weights' in metrics and isinstance(metrics['Weights'], str):
                    weights_list.append(metrics['Weights'])
...
def get_model(model_name):
    ...
    for weights in weights_list:
        # Ensure weights is a string before calling split()
        if isinstance(weights, str) and model_name == weights.split('/')[-2]:
